...
const LearnPage = ({ isDark }) => {
    ...
    return (<div className="min-h-screen pt-32 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {categories.map((category) => (<button key={category} className={`px-6 py-2 rounded-full text-sm font-medium transition-all duration-200 border ${category === 'All'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {articles.map((article, index) => (<article key={index} className={`group p-8 rounded-2xl backdrop-blur-sm border transition-all duration-300 hover:scale-105 cursor-pointer ${isDark
                ? 'bg-white/5 border-white/10 hover:bg-white/10'
                : 'bg-white border-gray-200 hover:bg-gray-50 hover:shadow-xl'}`}>
              <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border mb-6 ${(0, formatters_1.getCategoryStyle)(article.category, isDark)}`}>
                {article.category}
              </div>
              
              <h3 className={`text-xl font-bold mb-4 leading-tight group-hover:text-blue-500 transition-colors duration-200 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {article.title}
              </h3>
              ...
              <div className={`flex items-center justify-between text-sm border-t pt-4 ${isDark ? 'border-white/10 text-slate-400' : 'border-gray-200 text-slate-500'}`}>
                <div className="flex items-center gap-2">
                  <div className={`w-6 h-6 rounded-full bg-gradient-to-r ${index % 4 === 0 ? 'from-blue-500 to-purple-600' :
                index % 4 === 1 ? 'from-emerald-500 to-teal-600' :
                    index % 4 === 2 ? 'from-orange-500 to-red-600' :
                        'from-purple-500 to-pink-600'}`}></div>
        ...
        {/* Newsletter signup */}
        <div className={`mt-20 p-12 rounded-3xl backdrop-blur-sm border text-center ${isDark
          ...
          <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
            <input type="email" placeholder="Enter your professional email" className={`flex-1 px-4 py-3 rounded-xl border backdrop-blur-sm ${isDark
            ? 'bg-white/10 border-white/20 text-white placeholder-slate-400'
            : 'bg-white border-gray-200 text-gray-900 placeholder-slate-500'} focus:outline-none focus:ring-2 focus:ring-blue-500`}/>
